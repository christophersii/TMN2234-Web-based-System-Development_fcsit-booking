<?php
    ...
    $print_id= $_GET['print'];
    $sql = "select publicbookinginvoice.BookingNo, publicbookinginvoice.GuestID, publicbookinginvoice.RoomNo, room.RoomName, room.Date, room.Time, room.Capacity, room.Price, publicuser.username, publicuser.useremail from publicbookinginvoice inner join room on publicbookinginvoice.RoomNo = room.RoomNo inner join publicuser on publicbookinginvoice.GuestID = publicuser.GuestID where publicbookinginvoice.BookingNo= '$print_id'";
    ...
    while($rows = mysqli_fetch_array($result))
    {
        ...
        $pdf->Ln();
        $pdf->Cell(65, 10, 'Guest ID', 1, 0, 'C');
        $pdf->Cell(125, 10, $rows['GuestID'], 1, 0, 'L');
        $pdf->Ln();
        ...
        $pdf->Ln();
        $pdf->Cell(65, 10, 'Paid Price (RM)', 1, 0, 'C');
        $pdf->Cell(125, 10, $rows['Price'], 1, 0, 'L');
        $pdf->Ln();
        $pdf->Ln();
