<?php
  ...
  if($_SERVER['REQUEST_METHOD'] == 'POST'){
      $from= ($_POST['fromguestreport']);
      $until= ($_POST['untilguestreport']);
      ...
      if ($dbcon->connect_error){
        ...
        class PDF extends FPDF{
            ...
            function Header()
            {
                ...
                // Line break
                $this->Ln(14);
                $this->Cell(200,10,'Guest Booking Report',0,0,'C');
            }
        ...
        $sql = "select publicbookinginvoice.BookingNo, publicbookinginvoice.GuestID, publicbookinginvoice.RoomNo, room.RoomName, room.Date, room.Time, room.Capacity, room.Price, publicuser.username, publicuser.useremail from publicbookinginvoice inner join room on publicbookinginvoice.RoomNo = room.RoomNo inner join publicuser on publicbookinginvoice.GuestID = publicuser.GuestID WHERE room.Date BETWEEN '$from' AND '$until'";
        $result = mysqli_query($dbcon, $sql);

        $pdf->SetFont('Times', 'B', 8);
        $pdf->Ln();
        $pdf->SetFont('times', 'B', 8);
        $pdf->Cell(15, 10, 'Booking No', 1, 0, 'C');
        $pdf->Cell(13, 10, 'Guest ID', 1, 0, 'C');
        $pdf->Cell(30, 10, 'Guest Name', 1, 0, 'C');
        $pdf->Cell(30, 10, 'Guest Email', 1, 0, 'C');
        $pdf->Cell(13, 10, 'Room No', 1, 0, 'C');
        $pdf->Cell(30, 10, 'Room Name', 1, 0, 'C');
        $pdf->Cell(13, 10, 'Capacity', 1, 0, 'C');
        $pdf->Cell(18, 10, 'Date', 1, 0, 'C');
        $pdf->Cell(13, 10, 'Time', 1, 0, 'C');
        $pdf->Cell(15, 10, 'Price (RM)', 1, 0, 'C');
        ...
        while($rows = mysqli_fetch_array($result))
        {
            $pdf->Ln();
            $pdf->SetFont('times', 'B', 8);
            $pdf->Cell(15, 10, $rows['BookingNo'], 1, 0, 'C');
            $pdf->Cell(13, 10, $rows['GuestID'], 1, 0, 'C');
            $pdf->Cell(30, 10, $rows['username'], 1, 0, 'C');
            $pdf->Cell(30, 10, $rows['useremail'], 1, 0, 'C');
            $pdf->Cell(13, 10, $rows['RoomNo'], 1, 0, 'C');
            $pdf->Cell(30, 10, $rows['RoomName'], 1, 0, 'C');
            $pdf->Cell(13, 10, $rows['Capacity'], 1, 0, 'C');
            $pdf->Cell(18, 10, $rows['Date'], 1, 0, 'C');
            $pdf->Cell(13, 10, $rows['Time'], 1, 0, 'C');
            $pdf->Cell(15, 10, $rows['Price'], 1, 0, 'C');
...
<head>
    ...
    <title>Admin Guest Report</title>
...
<body>
    ...
    <div class="form_wrapper">
        ...
        <div class="title_container">
            <h2>Print Guest Report</h2>
        </div>
            <div>
                <form method="post">
                    <div> 
                        <span>From</span>
                        <input type="date" name="fromguestreport" placeholder="Date"  />
                    </div>
                    <div > 
                        <span>Until </span>
                        <input type="date" name="untilguestreport" placeholder="Date"  />
                    </div>
                    <input type="submit" name= "guestreport" Value="Print">
