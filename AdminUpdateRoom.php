...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if (empty($roomName)){
        ...
        echo "<script>window.open('AdminUpdateRoom.php','_self')</script>";
        exit();
    }else{
        $roomName = true;
    }
    ...
    if ($roomName == true && $date == true && $time == true && $capacity == true && $availability = true && $price == true){
      ...
      $update_id=$_GET['upd'];
      $stmt = $dbcon->prepare("UPDATE room SET RoomName=?, AdminID=?, Date=?, Time=?, Capacity=?, Price=?, Availability=? WHERE RoomNo= '$update_id'");
