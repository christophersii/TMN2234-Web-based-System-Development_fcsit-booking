...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if ($username == true &&  $Password == true && $ContactNo == true){
      ...
      if ($dbcon->connect_error){
          ...
          $stmt = $dbcon->prepare("UPDATE internaluser SET UserName=?, ContactNo=?, Password=? WHERE useremail= '".$_SESSION['UserEmail']."'");
          $stmt->bind_param('sis', $username, $ContactNo, $Password);
          $stmt->execute();
          echo "<script>alert('Update successful.')</script>";
          echo "<script>window.open('InternalProfile.php','_self')</script>";
      }
  }
  else{
    ...
    <div class="form_container">
      ...
      <div class="row clearfix">
        <form name="reg_form" id="reg_form" method="post";>
            ...
            <a class="button1" href="InternalProfile.php">Back</a>
      </div>
    </div>
  </div>
    <div class="footer">
