...
<body>
    <div class="form_wrapper">
        <div class="form_container">
            ...
            <div class="row clearfix">
                <h3>Don't have a guest account?</h3>
                <a class="button1" href="GuestRegistration.php">Sign Up</a>
            </div>
        </div>
    </div>
    <div class="form_wrapper">
...
{  
    ...
    $check_user="select * from publicuser WHERE UserEmail='$user_email'AND Password='$user_pass'";  
  
    $run=mysqli_query($dbcon,$check_user);  
  
    if(mysqli_num_rows($run))  
    {  
        echo "<script>window.open('GuestDashboard.php','_self')</script>";  
        $_SESSION['UserEmail']=$user_email; //here session is used and value of $user_email store in $_SESSION.  
    }  
    else  
    {  
      echo "<script>alert('Email or password is incorrect!')</script>";
      echo "<script>window.open('GuestLogin.php','_self')</script>";
    }
}  
?> 